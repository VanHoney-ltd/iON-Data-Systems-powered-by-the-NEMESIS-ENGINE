...
impl Agent for CerberusAgent {
    ...
    fn extract(ctx: &AgentCtx) -> Result<Vec<EvidenceRecord>> {
        ...
        match extract_contacts_internal(ctx) {
            ...
            Err(e) => {
                let warning = format!("Cerberus contacts extraction skipped: {e:#}");
                ctx.log(&warning);
        ...
        match extract_call_history(ctx) {
            ...
            Err(e) => {
                let warning = format!("Cerberus call history extraction skipped: {e:#}");
                ctx.log(&warning);
                warnings.push(warning);
            }
        }
        ...
        match extract_voicemail(ctx) {
            ...
            Err(e) => {
                let warning = format!("Cerberus voicemail extraction skipped: {e:#}");
                ctx.log(&warning);
                warnings.push(warning);
            }
        }
        ...
        match extract_messages_and_attachments(ctx) {
            ...
            Err(e) => {
                let warning = format!("Cerberus SMS/MMS extraction skipped: {e:#}");
                ctx.log(&warning);
                warnings.push(warning);
            }
        }
...
fn prepare_message_statement(conn: &rusqlite::Connection) -> Result<rusqlite::Statement<'_>> {
    let query = build_message_query(conn)?;
    conn.prepare(&query)
        .context("preparing Cerberus SMS/MMS message query")
}

fn build_message_query(conn: &rusqlite::Connection) -> Result<String> {
    let mut stmt = conn
        .prepare("PRAGMA table_info(message)")
        .context("inspecting Cerberus SMS/MMS message schema")?;
    let columns = stmt
        .query_map([], |row| row.get::<_, String>(1))
        .context("reading Cerberus SMS/MMS message schema")?
        .collect::<rusqlite::Result<BTreeSet<_>>>()
        .context("decoding Cerberus SMS/MMS message schema")?;

    if columns.is_empty() {
        anyhow::bail!("Cerberus SMS/MMS database is missing the message table");
    }
    ...
    let projections = [
        ...
        optional("date"),
        optional("date_read"),
        optional("date_delivered"),
        optional("is_from_me"),
        optional("is_read"),
        optional("is_delivered"),
        optional("is_sent"),
        optional("service"),
        optional("subject"),
        optional("cache_has_attachments"),
        optional("item_type"),
        optional("group_title"),
        "h.id AS phone_number".to_string(),
        "c.ROWID AS chat_id".to_string(),
        "c.chat_identifier".to_string(),
        "c.display_name".to_string(),
        optional("is_audio_message"),
        optional("reply_to_guid"),
        optional("balloon_bundle_id"),
    ]
    .join(",\n            ");
    let date_order = if columns.contains("date") {
        "m.date"
    } else {
        "NULL"
    };
    ...
    Ok(format!(
        ...
        ORDER BY COALESCE(c.ROWID, -m.ROWID), {date_order} ASC, m.ROWID ASC
        "#
    ))
}
...
fn extract_messages_and_attachments(
    ...
    let mut stmt = prepare_message_statement(&conn)?;
    ...
    let msg_rows = stmt
        ...
        .context("querying Cerberus SMS/MMS messages")?;
    ...
    for row in msg_rows {
        let (
            ...
            reply_to_guid,
            balloon_bundle_id,
        ) = row.context("decoding a Cerberus SMS/MMS message row")?;
...
fn load_attachment_rows(conn: &rusqlite::Connection) -> Result<Vec<CerberusAttachmentExport>> {
    let mut stmt = conn
        ...
        .context("preparing Cerberus SMS/MMS attachment query")?;
    ...
    let rows = stmt
        ...
        .context("querying Cerberus SMS/MMS attachments")?;

    rows.collect::<rusqlite::Result<Vec<_>>>()
        .context("decoding Cerberus SMS/MMS attachment rows")
}
...
#[cfg(test)]
mod tests {
    use super::prepare_message_statement;
    use rusqlite::Connection;

    fn create_legacy_message_schema(conn: &Connection, include_handle_table: bool) {
        conn.execute_batch(
            r#"
        ...
        .expect("create legacy message schema");

        if include_handle_table {
            conn.execute_batch(
                r#"
                CREATE TABLE handle (id TEXT NOT NULL);
                INSERT INTO handle (ROWID, id) VALUES (7, '+15551234567');
                "#,
            )
            .expect("create legacy handle schema");
        }

        conn.execute(
            "INSERT INTO message (ROWID, guid, date, handle_id) VALUES (1, 'legacy-guid', 42, 7)",
            [],
        )
    ...
    #[test]
    fn message_query_supports_schema_without_reply_to_guid() {
        let conn = Connection::open_in_memory().expect("open in-memory database");
        create_legacy_message_schema(&conn, true);

        let mut stmt = prepare_message_statement(&conn).expect("prepare legacy message query");
        let (id, guid, text, handle, reply_to_guid, balloon_bundle_id) = stmt
            .query_row([], |row| {
                Ok((
                    row.get::<_, i64>(0)?,
                    row.get::<_, String>(1)?,
            ...
            .expect("read legacy message");

        assert_eq!(id, 1);
        assert_eq!(guid, "legacy-guid");
        assert_eq!(text, None);
        assert_eq!(handle.as_deref(), Some("+15551234567"));
        assert_eq!(reply_to_guid, None);
        assert_eq!(balloon_bundle_id, None);
    }

    #[test]
    fn message_query_prepare_errors_are_propagated() {
        let conn = Connection::open_in_memory().expect("open in-memory database");
        create_legacy_message_schema(&conn, false);

        let error = match prepare_message_statement(&conn) {
            Ok(_) => panic!("missing handle table should fail query preparation"),
            Err(error) => error,
        };
        let message = format!("{error:#}");

        assert!(message.contains("preparing Cerberus SMS/MMS message query"));
        assert!(message.contains("no such table: handle"));
    }
}
