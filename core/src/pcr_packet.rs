...
fn run() -> Result<()> {
    ...
    let docs = vec![
        ...
        ("Exhibit_B_Financial_Extortion_Timeline", generate_exhibit_b(&data)),
        ("Exhibit_C_Complainant_Contradictions", generate_exhibit_c(&data)),
        ("Exhibit_D_Relationship_Evidence", generate_exhibit_d(&data)),
        ("Exhibit_E_Judicial_Weaponization", generate_exhibit_e(&data)),
    ];
...
impl EvidenceData {
    fn load(case_root: &Path) -> Result<Self> {
        ...
        // Load CashApp
        let cash_path = case_root.join("logs").join("cash_app_report_1768240721597.csv");
        if cash_path.exists() {
...
fn try_weasyprint(html_path: &Path, pdf_path: &Path) -> bool {
        ...
        .output()
    {
        ...
        Ok(out) => {
            eprintln!("weasyprint failed: {}", String::from_utf8_lossy(&out.stderr));
            false
        }
        Err(e) => {
...
fn generate_exhibit_a(data: &EvidenceData) -> Result<String> {
    ...
    body.push_str(&case_header_block("Exhibit A: Borderline Personality Disorder Pattern Analysis"));
...
<tbody>"#);
    ...
    let block_keywords = [
        "block", "blocked", "unblock", "changed my number", "new number",
        "don't contact me", "leave me alone", "fuck off",
    ];
    let mut rows = 0;
...
fn generate_exhibit_b(data: &EvidenceData) -> Result<String> {
    ...
    body.push_str(&case_header_block("Exhibit B: Financial Extortion Timeline"));
...
<tbody>"#);
    ...
    let coerce_keywords = [
        "block", "unblock", "ignore", "apologize", "stop", "don't", "need",
        "please", "hard", "making", "love", "fuck", "shit",
    ];
    let mut rows = 0;
...
fn generate_exhibit_c(data: &EvidenceData) -> Result<String> {
    ...
    body.push_str(&case_header_block("Exhibit C: Complainant Statement Contradictions"));
...
<tbody>"#);
    ...
    let intimacy_keywords = [
        "love you", "baby", "pregnant", "engagement", "ring", "marry", "marriage",
        "sex", "sexy", "miss you", "need you", "want you",
    ];
    let mut rows = 0;
    for sms in data.sms.iter().rev() {
        let lower = sms.text.to_lowercase();
...
fn generate_exhibit_d(data: &EvidenceData) -> Result<String> {
    ...
    body.push_str(&case_header_block("Exhibit D: Domestic Relationship Evidence"));
...
<tbody>"#);
    ...
    let rel_keywords = [
        "engagement", "ring", "marry", "wedding", "pregnant", "baby", "our baby",
        "birth certificate", "hospital", "give birth", "luna", "ellie",
    ];
    let mut rows = 0;
    for sms in data.sms.iter().rev() {
        let lower = sms.text.to_lowercase();
...
fn generate_exhibit_e(data: &EvidenceData) -> Result<String> {
    ...
    body.push_str(&case_header_block("Exhibit E: Judicial Weaponization Analysis"));
